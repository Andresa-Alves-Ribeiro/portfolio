...
const Header = () => {
  ...
  return (
    <header className={`fixed top-0 left-0 w-full z-[1000] transition-all duration-300 ${
      isDarkMode 
        ? 'bg-[rgba(255,182,217,0.9)] border-b border-primary shadow-[0_0_20px_rgba(255,105,180,0.2)]' 
        : 'bg-[rgba(255,255,255,0.95)] backdrop-blur-[10px] border-b-2 border-dashed border-primary shadow-[0_0_20px_rgba(255,105,180,0.1)]'
    } ${scrolled ? 'py-2' : 'py-4'}`}>
      <ul className="flex justify-between items-center list-none relative max-w-[1400px] mx-auto w-full px-4 md:px-8">
        ...
        <div className={`md:flex gap-4 md:gap-8 relative after:content-[''] after:absolute after:top-1/2 after:-right-5 after:w-px after:h-[30px] after:bg-gradient-to-b after:from-transparent after:via-[rgba(147,112,219,0.5)] after:via-[rgba(135,206,235,0.5)] after:via-[rgba(255,105,180,0.5)] after:to-transparent after:-translate-y-1/2 after:animate-pulse md:after:block after:hidden ${
          isMenuOpen 
            ? 'fixed top-[70px] left-0 w-full h-[calc(100vh-70px)] bg-white-cream backdrop-blur-[10px] p-8 flex-col gap-8 border-t-2 border-dashed border-primary translate-x-0 overflow-y-auto z-[1000]' 
            : 'hidden md:flex translate-x-[-100%]'
        }`}>
          <li className="m-0 relative">
            <a 
              href="#about" 
              className={`text-primary-dark no-underline font-handwriting font-bold transition-all duration-300 relative px-4 py-2 rounded bg-transparent tracking-widest text-sm before:content-[''] before:absolute before:bottom-0 before:left-1/2 before:w-0 before:h-0.5 before:bg-primary before:transition-all before:duration-300 before:-translate-x-1/2 hover:text-primary hover:drop-shadow-[0_0_10px_rgba(255,105,180,0.5)] hover:before:w-4/5`}
